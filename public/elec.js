...
addNote = () => {
    ...
    if (title.length == 0) {
        ...
        if (localStorage.getItem('elec') == null) {
            data = []
        }
        else {
            data = JSON.parse(localStorage.getItem('elec'))
        }
        ...
        localStorage.setItem('elec', data);
...
/* Perfect */
showNotes = () => {
    let notes = localStorage.getItem('elec');
...
/* Perfect */
deleteNotes = (id) => {
    let data = JSON.parse(localStorage.getItem('elec'));
    data.splice(id, 1);
    data = JSON.stringify(data);
    localStorage.setItem('elec', data);
...
<head>
    ...
    <meta name="keywords" content="todo, list, reminder, elec">
