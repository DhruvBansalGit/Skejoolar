...
addNote = () => {
    ...
    if (title.length == 0) {
        ...
        if (localStorage.getItem('define') == null) {
            data = []
        }
        else {
            data = JSON.parse(localStorage.getItem('define'))
        }
        ...
        localStorage.setItem('define', data);
...
/* Perfect */
showNotes = () => {
    let notes = localStorage.getItem('define');
...
/* Perfect */
deleteNotes = (id) => {
    let data = JSON.parse(localStorage.getItem('define'));
    data.splice(id, 1);
    data = JSON.stringify(data);
    localStorage.setItem('define', data);
...
<head>
    ...
    <meta name="keywords" content="todo, list, reminder, define">
